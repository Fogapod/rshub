...
pub trait Tab {
    fn name(&self, app: &App) -> String;
}

pub(crate) struct ServersTab {}

impl Tab for ServersTab {
    fn name(&self, app: &App) -> String {
        let servers = &app.servers;

        format!("servers [{}]", servers.read().count())
    }
}

pub(crate) struct InstallationsTab {}

impl Tab for InstallationsTab {
    fn name(&self, app: &App) -> String {
        let servers = &app.servers;

        format!("installations [{}]", servers.read().count())
    }
}

pub(crate) struct CommitsTab {}

impl Tab for CommitsTab {
    fn name(&self, _: &App) -> String {
        "commits".to_owned()
    }
}
    ...
    B: Backend,
{
    ...
    let rows = servers_to_be_sorted.iter().map(|s| {
        ...
        Row::new(vec![
            s.data.name.clone(),
            s.data.build.to_string().clone(),
            s.data.map.clone(),
            s.data.players.to_string().clone(),
        ])
        .style(style)
    });
